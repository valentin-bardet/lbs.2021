    ...
    public function addCommand(Request $req, Response $resp, array $args): Response
    {
        ...
        if (!isset($command_data['items'])) {
            $resp->getBody()->write(json_encode([
                "type" => "error",
                "error" => "400",
                "message" => 'Valeur manquante : items',
            ]));
            return $resp;
        }
        ...
        try {
            ...
            foreach ($command_data['items'] as $item) {
                $i = new Item();
                $i->uri = filter_var($item['uri'], FILTER_SANITIZE_STRING);
                $i->libelle = filter_var($item['libelle'], FILTER_SANITIZE_STRING);
                $i->tarif = filter_var($item['tarif'], FILTER_SANITIZE_NUMBER_FLOAT);
                $i->quantite = filter_var($item['q'], FILTER_SANITIZE_NUMBER_INT);
                $i->command_id = $c->id;
                $c->montant = $c->montant + ($i->tarif * $i->quantite);
                $i->save();
            }
            $c->save();
